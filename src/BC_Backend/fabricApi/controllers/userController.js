...
exports.resetPassword = async (req, res) => {
  try {
    const {common_name, username, citizen_id} = req.body;

    if (!common_name || !username || !citizen_id) {
      return res.status(400).json({success: false, message: 'Thiếu thông tin'});
    }

    const result = await userService.generateNewPassword(
      common_name,
      username,
      citizen_id,
    );
...
exports.getDegreesByUser = async (req, res) => {
  try {
    const {id} = req.params;
    const data = await userService.getDegrees(id);
    console.log(data);
    res.status(200).json(data);
  } catch (error) {
    res.status(500).json({success: false, message: error.message});
